...
private static boolean sendGetRequestWithTimeout(String directory, int port) throws Exception {
    Socket socket = null;
    try {
        socket = new Socket();
    ...
    private static boolean verifyPutLamportClocks(String directory, int port) throws Exception {
        ...
        while ((line = in1.readLine()) != null) {
            if (line.contains("Lamport-Clock =") || line.contains("Lamport clock after PUT request:")) {
    ...
    //         try (Socket testSocket = new Socket("localhost", LAMPORT_TEST_PORT)) {
    //             System.out.println("Server is accepting connections...");
    //         }
            
    //         // First PUT request
    //         System.out.println("Sending first PUT request...");
    //         Integer clock1 = sendPutRequestWithTimeout(LAMPORT_TEST_PORT, 1, 10000); // Increased timeout
    //         if (clock1 == null) {
    //             System.out.println("Warning: First PUT request did not return a Lamport clock value");
    //         } else {
    //             System.out.println("First PUT request successful, clock value: " + clock1);
    //         }
    ...
    //         // Second PUT request
    //         System.out.println("Sending second PUT request...");
    //         Integer clock2 = sendPutRequestWithTimeout(LAMPORT_TEST_PORT, 2, 10000); // Increased timeout
    //         if (clock2 == null) {
    //             System.out.println("Warning: Second PUT request did not return a Lamport clock value");
    //         } else {
    //             System.out.println("Second PUT request successful, clock value: " + clock2);
    //         }
    ...
    //             }
    //         } else {
    //             String errorMsg = String.format("Could not retrieve Lamport clock values (clock1=%s, clock2=%s)", clock1, clock2);
    //             result.addError(errorMsg);
    //             result.failedRequests++;
    //         }
    ...
    private static Integer extractLamportClockFromLine(String line) {
        try {
            // Try all possible formats
            if (line.contains("Lamport-Clock = ")) {
                return Integer.parseInt(line.split("Lamport-Clock = ")[1].trim());
            } else if (line.contains("Lamport-Clock: ")) {
                return Integer.parseInt(line.split("Lamport-Clock:")[1].trim());
            } else if (line.contains("lamport-clock = ")) {
                return Integer.parseInt(line.split("lamport-clock = ")[1].trim());
            } else if (line.contains("Lamport-Time: ")) {
                return Integer.parseInt(line.split("Lamport-Time:")[1].trim());
            }
        } catch (NumberFormatException e) {
            System.err.println("Error parsing Lamport clock value from line: " + line);
        }
        return null;
    }
    
    private static Integer sendPutRequestWithTimeout(int port, int clockValue, int timeoutMillis) {
        Socket socket = null;
        try {
            // Create socket with timeout
            socket = new Socket();
            socket.connect(new InetSocketAddress("localhost", port), timeoutMillis);
            ...
            String requestBody = TEST_JSON;
            
            // Send PUT request with headers - match Nur's format
            System.out.println("Sending PUT request to server...");
            ...
            writer.write("Lamport-Clock = " + clockValue + "\r\n"); // Using equals sign format
            writer.write("\r\n");  // End of headers
            ...
            while ((System.currentTimeMillis() - startTime) < timeoutMillis) {
                ...
                // Try to extract Lamport clock using helper method
                Integer clockFromLine = extractLamportClockFromLine(line);
                if (clockFromLine != null) {
                    responseClock = clockFromLine;
                    System.out.println("Found Lamport clock value: " + responseClock);
                    break;
                }
                
                // Break if we hit end of headers
                if (line.trim().isEmpty()) {
                    System.out.println("End of headers reached");
                    break;
                }
            }
            ...
            if (responseClock == null) {
                ...
                System.err.println("No Lamport clock found in response");
            }
            
            return responseClock;
            
        } catch (SocketTimeoutException e) {
            System.err.println("Socket timeout while reading response");
            return null;
        } catch (Exception e) {
            System.err.println("Error in PUT request: " + e.getMessage());
            e.printStackTrace();
            return null;
        } finally {
            if (socket != null) {
